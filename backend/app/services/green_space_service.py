...
from ..models import GreenSpace, MaintenanceRecord, MaintenanceTask, PlantReplacement, WasteRecord
...
class GreenSpaceService(BaseService):
    ...
    def delete(cls, obj_id, force=False):
        ...
        counts = {
            ...
            .scalar()
            or 0,
            "waste_record": db.session.query(func.count(WasteRecord.id))
            .filter(WasteRecord.green_space_id == space.id)
            .scalar()
            or 0,
        }
        if sum(counts.values()) and not force:
            raise ConflictError(
                ...
                "绿植更换记录 {plant_replacement} 条、废弃物处置记录 {waste_record} 条，"
