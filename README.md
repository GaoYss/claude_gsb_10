...
| 废弃物处置 | `/green-wastes` | 废弃物产生登记（来源绿地、类型、数量、产生环节）与处置补录（粉碎还田/外运消纳/资源利用、运输车辆、接收去向、处置日期）、按月份与绿地汇总产生量/处置量、产生量与处置量对不上的差异单独提示 |
...
│   │   │   ├── common.py        # 链式字段校验器
│   │   │   ├── filters.py       # 列表过滤条件
│   │   │   └── green_space.py / maintenance_task.py / maintenance_record.py / plant_replacement.py / green_waste.py
...
│   │   └── views/               # 视图：dashboard / green-space / task / record / replacement / waste
...
首次启动会自动建表；`SEED_DEMO_DATA=true` 时会写入一批演示数据（7 处绿地、16 条任务、22 条养护记录、8 条更换记录、约 15 条废弃物处置记录）。停止与清理：
...
| GET/POST | `/green-wastes` | 废弃物台账列表（`green_space_id`/`waste_type`/`disposal_method`/`status`/`month`/日期区间，返回汇总） / 登记废弃物产生与处置 |
| GET/PUT/DELETE | `/green-wastes/{id}` | 详情（含暂存/已处置状态与待处置量） / 更新（补录处置） / 删除 |
| GET | `/green-wastes/summary` | 废弃物汇总（总量、按处置方式/类型、按月份 `by_month`、按绿地 `by_green_space`、差异 `unbalanced`） |
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口 |

## 六、业务规则

1. **业务编号**：绿地 `GS-年份-序号`（如 `GS-2026-0001`），任务 `MT-YYYYMMDD-序号`，养护记录 `MR-YYYYMMDD-序号`，更换记录 `PR-YYYYMMDD-序号`，废弃物记录 `WA-YYYYMMDD-序号`；留空自动生成，唯一约束冲突时自动重试，编号创建后不可修改。
...
6. **废弃物处置台账**：同一记录先登记产生（来源绿地、类型、产生量、产生日期），再补录处置（处置方式、处置量、运输车辆、接收/去向单位、处置日期）。处置方式分**粉碎还田 / 外运消纳 / 资源利用**；处置方式与处置日期须成对补全，处置量留空默认等于产生量（全部处置）且不得大于产生量，处置日期不得早于产生日期。未补录处置即为**暂存待处置**，待处置量 = 产生量 − 处置量，按产生月份与来源绿地两个维度汇总，差异 >0 单独提示。
7. **删除保护**：删除绿地时若已存在任务/记录/更换/废弃物数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；删除任务时养护记录默认保留（解除关联），避免养护履历丢失；删除养护记录时关联废弃物的养护记录外键置空（SET NULL）。
...
| `green_waste` | 废弃物处置台账 | `waste_no`(唯一)、`green_space_id`、`maintenance_record_id`(可空)、`waste_type`、`quantity`、`unit`、`produce_date`、`source_detail`；处置侧 `disposal_method`、`disposal_quantity`、`disposal_date`、`vehicle_no`、`receiver`、`disposal_note` |

绿地删除时任务/记录/更换/废弃物级联清理；任务与养护记录之间、养护记录与更换记录/废弃物记录之间为可空外键（`SET NULL`），保证养护履历可独立留存。废弃物「是否已处置」不入库，由 `disposal_method` 与 `disposal_date` 是否同时有值派生，待处置量 = 产生量 − 已处置量。

## 八、测试

```bash
cd backend
python -m pytest              # 69 个用例：接口、校验、跨模块规则、端到端流程、废弃物台账与差异汇总
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、删除保护与强制删除、统计聚合口径一致性、**废弃物产生登记与处置补录（处置量约束、成组必填、日期约束）、按月份/绿地的产生量·处置量·待处置差异汇总、演示数据自洽性**。
