...
面向城市绿化养护单位的一体化记录系统：以**绿地台账**为主线，串联**养护任务登记**、**养护记录录入**、**绿植更换记录**与**绿化废弃物处置台账**，并提供养护总览看板。
...
| 废弃物处置 | `/waste` | 废弃物台账（来源绿地、类型、产生量/体积、运输车辆与去向）、粉碎还田/外运消纳/资源利用分类、按月份与绿地对账产生量与处置量、差额单独提示 |
...
│   │   │   ├── common.py        # 链式字段校验器
│   │   │   ├── filters.py       # 列表过滤条件
│   │   │   └── green_space.py / maintenance_task.py / maintenance_record.py / plant_replacement.py / waste_record.py
...
首次启动会自动建表；`SEED_DEMO_DATA=true` 时会写入一批演示数据（7 处绿地、15 条任务、22 条养护记录、8 条更换记录、23 条废弃物处置记录）。停止与清理：
...
| GET/POST | `/waste-records` | 废弃物台账列表（`green_space_id`/`waste_type`/`disposal_method`/关键字/产生日期区间，返回汇总） / 登记处置记录 |
| GET/PUT/DELETE | `/waste-records/{id}` | 详情 / 更新 / 删除 |
| GET | `/waste-records/summary` | 处置汇总（分单位合计、按废弃物类型、按处置方式） |
| GET | `/waste-records/monthly` | 月度对账（按产生月份 × 绿地 × 单位汇总产生量/处置量/差额，差额行单独列入 `mismatches`） |
| GET | `/statistics/dashboard` | 看板聚合数据（总览 + 分布 + 趋势 + 榜单 + 提醒 + 最近动态） |
| GET | `/statistics/overview` `/distributions` `/trends` `/ranking` `/reminders` | 看板分项接口 |

## 六、业务规则

1. **业务编号**：绿地 `GS-年份-序号`（如 `GS-2026-0001`），任务 `MT-YYYYMMDD-序号`，养护记录 `MR-YYYYMMDD-序号`，更换记录 `PR-YYYYMMDD-序号`，废弃物处置记录 `WR-YYYYMMDD-序号`；留空自动生成，唯一约束冲突时自动重试，编号创建后不可修改。
...
4. **日期约束**：养护日期、更换日期、废弃物产生日期不得早于绿地建成日期；废弃物处置日期不得早于产生日期。
5. **金额核算**：更换金额 = 数量 × 单价，由后端统一计算；未填单价时金额留空，前端提示补录。
6. **废弃物对账**：处置量不得大于产生量；处置方式为「外运消纳」时必须登记运输车辆与去向（粉碎还田、资源利用可选填）；产生量 − 处置量为待处置量，`/waste-records/monthly` 按产生月份 × 绿地 × 计量单位汇总产生量与处置量，差额不为零的行单独放入 `mismatches` 返回，前端以警示列表单独提示。
7. **删除保护**：删除绿地时若已存在任务/记录/更换/废弃物数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；删除任务时养护记录默认保留（解除关联），避免养护履历丢失。
...
| `waste_record` | 绿化废弃物处置台账 | `record_no`(唯一)、`green_space_id`、`waste_type`、`generate_date`、`quantity`、`unit`、`disposal_method`、`disposed_quantity`、`disposed_date`、`transport_vehicle`、`destination` |

绿地删除时任务/记录/更换/废弃物级联清理；任务与养护记录之间、养护记录与更换记录之间为可空外键（`SET NULL`），保证养护履历可独立留存。

## 八、测试

```bash
cd backend
python -m pytest              # 67 个用例：接口、校验、跨模块规则、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、废弃物处置量与日期约束、外运消纳必填项、月度对账与差额提示、删除保护与强制删除、统计聚合口径一致性、演示数据自洽性。
